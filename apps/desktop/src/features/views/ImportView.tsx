import { useRef, useMemo, useState, useCallback, useEffect, type DragEvent } from "react";
...
export function ImportView() {
  ...
  // Auto-select a default morphology library if none is active
  useEffect(() => {
    if (!activeCollectionId && BUNDLED_COLLECTIONS.length > 0) {
      setActiveCollectionId(BUNDLED_COLLECTIONS[0].id);
    }
  }, [activeCollectionId, setActiveCollectionId]);

  // Zero-Click Automation Payload
  useEffect(() => {
    if (canQuickGenerate && activeCollectionId && !hasAutoGenerated.current) {
      hasAutoGenerated.current = true;
      // Synthesize mesh seamlessly
      generateDesign();
    }
    // Release lock if assets are invalidated
    if (!canQuickGenerate) {
      hasAutoGenerated.current = false;
    }
  }, [canQuickGenerate, activeCollectionId, generateDesign]);
              ...
              <div>
                <div className="import-tooth-chips">
                  ...
                  <div
                    ...
                    onClick={() => toothRef2.current?.click()}
                  >
                    <input
                      ref={toothRef2}
                      type="file"
        ...
        <div className="import-action-zone" data-testid="import-action-zone">
          ...
          {validation.ok && (
            <div className="import-generate-card">
              ...
              <div style={{ padding: "16px", textAlign: "center", background: "rgba(0,180,216,0.1)", borderRadius: 8, marginTop: 12 }}>
                <span className="import-status-banner" style={{ background: "transparent", border: "none" }}>
                  <svg className="spinner" width="16" height="16" viewBox="0 0 24 24" fill="var(--accent)">
                     <path d="M12 4V2C6.48 2 2 6.48 2 12h2c0-4.41 3.59-8 8-8z" />
                  </svg>
                  <span style={{ color: "var(--accent)", fontWeight: 600 }}>Zero-Click processing active... synthesizing geometry</span>
                </span>
              </div>
            </div>
          )}
